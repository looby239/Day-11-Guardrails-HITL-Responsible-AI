...
class OutputGuardrailPlugin(base_plugin.BasePlugin):
    ...
    async def after_model_callback(
        ...
        *,
        ...
        # Step 1: Check for PII and sensitive content
        filter_result = content_filter(response_text)
        if not filter_result["safe"]:
            redacted_content = self._create_safe_response(filter_result["redacted"])
            llm_response.content = redacted_content
            self.redacted_count += 1
            response_text = filter_result["redacted"]

        # Step 2: Check with LLM judge if enabled
        if self.use_llm_judge:
            judge_res = await llm_safety_check(response_text)
            if not judge_res.get("safe", True):
                safe_message = "Xin lỗi, phản hồi này không thể hiển thị do vi phạm chính sách an toàn."
                blocked_content = self._create_safe_response(safe_message)
                llm_response.content = blocked_content
                self.blocked_count += 1

        return llm_response

    def _create_safe_response(self, text: str) -> types.Content:
        """Create a Content object with a safe message."""
